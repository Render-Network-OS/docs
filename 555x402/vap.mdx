...
1.  **Off-Chain Aggregation**: The VAP Verifier maintains a session ledger in Redis. It verifies signatures and the hash chain (Nonce `N` must follow `N-1`).
...
3.  **On-Chain Settlement**: When the session ends, the Verifier submits a Settlement Transaction to Solana.
...
VAP v2 research is complete with 234 tests passing and all 5 attack trees validated as SECURE. This section documents the validated design that will replace v1.
...
*   **Sybil Attack**: ROI -81.7%. Max advantage 1.80x at 100 identities (logarithmic stake weight caps returns).
*   **Fabrication Attack**: ROI -179.7%.
*   **ContributionProof**: 296-byte proof format, entity-agnostic verification.
