...
**Wave verdict:** PARTIAL (Tier 1 deploy + safety-control gates PASS on both chains; Phase E Tier 2 CCTP-only burn-side proof PASS on Fuji + Amoy with `complete` Iris attestations; Tier 1 CCTP RT DEFERRED_PENDING_USDC_FAUCET on the Sepolia receive leg)
...
The remaining DEFERRED entry (Tier 1 CCTP RT on the Sepolia receive leg and the optional Tier 2 Sepolia mint) is honest about its scope; no synthetic burn tx or attestation cited.
...
**Partial PASS.** Real V4.1 canonical EVM deployed and verified on both Tier 1 testnets (Sepolia + Base Sepolia) via Circle SCP + Gas Station, with three of four contract-level acceptance gates PASS per chain (pause + unpause, direct grantRole rejection, propose + early-execute revert). Tier 2 CCTP-only proof (Fuji + Amoy) is now fully PASS on both chains: structural readiness + USDC approve + `depositForBurn` ABI-reachability + on-chain burn + Iris `complete` attestation, with the SCA's `balanceOf` decreasing exactly `1.0 USDC` per chain and Gas Station sponsoring every userOp (post-burn `eth_getBalance(SCA)` stayed `0` on both chains). The remaining `DEFERRED_PENDING_USDC_FAUCET` item is the Tier 1 CCTP V2 zap-and-bridge round-trip on the Sepolia receive leg; the optional Tier 2 Sepolia `receiveMessage` mint is also deferred to avoid SCA nonce contention with the Phase D Sepolia receive run. Tier 3 mainnet-fork compat is complete on AVAX + Polygon (5 + 5 tests against block-pinned live state). W1 ships as partial-PASS; the deferred items are scoped to a focused Sepolia-receive sweep that consumes the existing attested messages without redoing any code work.
