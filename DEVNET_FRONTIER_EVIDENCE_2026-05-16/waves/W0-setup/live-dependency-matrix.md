...
| sw4p-backend HTTP API | `api.sw4p.io` / `staging-api.sw4p.io` / Railway `sw4p-backend` | curl `/health` and `/sdk/v1/limits` | `protocol-endpoint-discovery.md`, `phase-5-baseline-deferred.md` | **BLOCKED.** No reachable real sw4p-backend endpoint at probe time. Mock-only endpoint detected but forbidden under ZERO-MOCKS. Unblock criteria in `phase-5-baseline-deferred.md`. **Gates W1 through W8.** |
...
W0.a probes that ran in Phase 2 of W0 all PASS. Phase 3 (AWS landing) is complete: sw4p.io is already serving from AWS EKS, so no Cloudflare origin edit was needed. Phase 4 (Circle gas sponsor) is complete with DEFERRED outcome: Circle Gas Station fit NOT CONFIRMED; Kora retained; unblock criteria documented in `circle-sponsor-deferral.md`. Phase 5 (baseline CCTP V2 round-trip) is complete with BLOCKED outcome: no reachable real sw4p-backend endpoint discovered; all protocol-endpoint-discovery probes returned 404, 503, or NXDOMAIN; mock-only endpoint detected but forbidden under ZERO-MOCKS; unblock criteria documented in `phase-5-baseline-deferred.md`. **This blocker gates W1 through W8.** The W1 tier roster and the W2 Phase 2 path are locked from probe data and ready to inform the W1 + W2 plan writers, but W1+ execution cannot proceed without backend restoration.

This matrix is v1 Phase 5 update; v2 (all rows finalized + acceptance criteria signed off) is written after W0 fully closes, as part of `acceptance.md`.
