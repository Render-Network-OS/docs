...
**Hyperlink** is the programmable participation interface for the Render Network Protocol. Every link is a smart payment surface that resolves to an on-chain action — working anywhere on the web.
...
### Multi-Chain
Links resolve across Solana, Base, and Polygon via [AGG](/protocol/agg) routing. Users pay with whatever they have; the protocol handles the rest.
...
3. **The Event**:
    - Payment is verified by [AGG](/protocol/agg).
    - [VAP](/protocol/vap) sends a `SPAWN_ENEMY` event to the Streamer's game client.
