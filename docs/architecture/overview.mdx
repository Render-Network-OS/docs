...
icon: "sitemap"
description: "How distribution, participation, verification, settlement, yield, and operations fit together across Render Network Protocol."
---

<Info>**Status: Live** — This system is operational.</Info>

**Render Network Protocol, or RNDRNTWRK, is the economic operating system for human and agent media. At a system level, it is organized into six operational layers: distribution, participation, verification, settlement, yield, and operations.**
...
The yield layer is where settled cross-chain volume is converted back into stake-bearing yield for the participants who supplied the liquidity and the coordination capital.

This layer is responsible for:
- Distributing real-fee yield from sw4p routing fees to liquidity providers
- Distributing $555 incentive yield, labelled separately from the real-fee headline
- Boosting rewards for locked-$555 holders
- Growing protocol-owned liquidity on its own depth
- Anti-wash enforcement on routed volume so rewards never compound on synthetic flow

**sw4p Earn** sits here. It is the yield surface that turns settled volume into network ownership instead of letting it leave as one-shot fee revenue.
...
These implementation surfaces are important, but they should be understood as implementation detail inside the larger six-layer system.
