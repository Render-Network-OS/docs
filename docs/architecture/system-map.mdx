...
graph TD
    ...
    subgraph Blockchain ["Solana"]
        Token["$555 SPL Token"]
        SW4P_Bridge["sw4p settlement"]
        SwapRouting["SPL swap routing"]
        Gasless["sw4p gasless"]
    end
    ...
    AGG_Svc -->|Swap| SwapRouting
    AGG_Svc -->|Gasless| Gasless
```
...
*   **sw4p Bridge**: Cross-chain settlement interface for native USDC across supported chains.
...
*   **sw4p settlement**: Native USDC settlement across supported chains, USDT corridor including Tron, 15-state lifecycle with checkpointed recovery.
*   **SPL swap routing**: Best-execution routing for any-token-in / USDC-out on Solana.
*   **sw4p gasless**: Built-in transaction sponsorship on Solana for frictionless user experience.
